...
public class CrawlerWorker extends Thread {
    ...
    @Override
    public void run() {
        Spider spider = Spider.create(pageHandler).addUrl(crawlerItem.getLinks().toArray(new String[]{})).
        ...
        while (!spider.isExitWhenComplete()){
            ...
            spider.getSite().setUserAgent(USER_AGENTS[Math.round(new Random().nextInt(USER_AGENTS.length))]);
        }
